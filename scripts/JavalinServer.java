...
public class JavalinServer {
    public static void main(String[] args) {
        var app = Javalin.create(config -> config.useVirtualThreads=false)
            ...
            .post("/posts/", ctx -> ctx.result("{\"code\": \"OK\"}").contentType("application/json") )
            .get("/posts/{id}", ctx -> ctx.result("{\"code\": \"OK\"}").contentType("application/json") )
            .put("/posts/{id}", ctx -> ctx.result("{\"code\": \"OK\"}").contentType("application/json") )
            .delete("/posts/{id}", ctx -> ctx.result("{\"code\": \"OK\"}").contentType("application/json") )
