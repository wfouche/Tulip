...
public class LlqHistogram {
   ...
   * quantized using {@link #llq(long)}. This value is used for display formatting of sub-nanosecond
   * buckets.
   */
  ...
  /**
   * Quantize a raw value {@code n} into the histogram's low-latency quantization bucket. The method
   * reduces resolution for large values while preserving readable scale transitions (ns → μs → ms →
   * s).
   *
  ...
  /**
   * Compute the average (mean) value across all recorded observations. The returned value is based
   * on the quantized bucket values and their counts.
   *
  ...
  /**
   * Estimate a percentile value (by quantized bucket) using the inclusive definition: the smallest
   * value whose cumulative count is >= the target rank.
   *
  ...
  /**
   * Compute the population standard deviation of recorded values (based on quantized bucket values
   * and counts). Returns 0.0 when fewer than two observations exist.
   *
  ...
  public String toHtmlString() {
    ...
    for (int i = qCounts.length - 1; i >= 0; i--) {
      if (qCounts[i] != 0) {
        ...
        // Percentile
        htmlString.append(String.format(Locale.US, "    <td>%.6f</td>\n", 1.0 - 1.0 * av / nv));
        ...
        if (percentage >= 1.0) {
          htmlString.append(
              String.format(Locale.US, "    <td><mark>%.3f</mark></td>\n", percentage));
        } else {
  ...
  /**
   * Populate this histogram from a JSON string where keys are quantized values (as numbers) mapped
   * to counts. Existing counts are cleared before loading.
   *
  ...
  /**
   * Serialize the non-zero buckets to a compact JSON string mapping quantized value → count. Note:
   * this method constructs JSON manually; for robust serialization prefer {@link
